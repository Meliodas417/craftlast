...
class Explorer:
    ...
    def run(self):
        ...
        while self.f_target - self.f_prev_target > 0.001:  # still found a better solution
            ...
            while self.current_src_index < len(self.src_events):
                ...
                if time.time() - self.start_time > 1800:  # 30 minutes
                    print('Time limit exceeded. Stop the exploration.')
    ...
    def fitness(events):
        ...
        oracle_scores = [float(e['score']) if e['score'] is not None else 0.0 for e in events if
                         e['event_type'] == 'oracle']
